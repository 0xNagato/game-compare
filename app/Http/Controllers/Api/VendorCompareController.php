...
class VendorCompareController extends Controller
{
    public function __invoke(Request $request): JsonResponse
    {
        ...
        $cacheKey = sprintf('vendors:%d:%s:%s', $productId, $regionsHash, $includeTax ? 'tax' : 'notax');

        $payload = Cache::get($cacheKey);

        if (! $payload) {
            $payload = Cache::lock("lock:{$cacheKey}", 10)->block(3, function () use ($cacheKey, $productId, $regions, $includeTax) {
